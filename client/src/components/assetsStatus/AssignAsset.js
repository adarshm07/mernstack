...
const AddUser = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    axios.post("http://localhost:4000/assetsStatus", user)
    .then(res => res.data.assetID)
    .then(assetID => axios.get(`http://localhost:4000/assets/${assetID}`).then(res => res))
    .then(res => {
    ...
    <div>
      ...
      <div className="w-75 mx-auto shadow p-5">
        ...
        <form onSubmit={e => onSubmit(e)}>
          ...
          <button type='submit'className="btn btn-primary btn-block">Assign Asset</button>
        </form>
      </div>
    </div>
    </div>
